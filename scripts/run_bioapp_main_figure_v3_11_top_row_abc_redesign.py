...
OUT = ROOT / "visualizations" / "bioapp_experiment" / "bioapp_main_figure_v3_11_top_row_abc_redesign"
...
def d2_distribution_diagnostics(values: pd.Series) -> dict[str, Any]:
    ...
    return {
        "phase": "BioApp Main Figure V3.11 top-row A/B/C redesign",
...
def panel_a(ax: plt.Axes) -> None:
    ...
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    xs = [0.11, 0.36, 0.61, 0.86]
    labels = [
        ("Frozen CTA\nendpoint", "evaluation-only", PALETTE["endpoint_positive"]),
        ("Immune-all\ndropout", "ST unchanged", "#6E6E6E"),
        ("Baseline vs\nSVTuner", "forced / withheld", PALETTE["prevented_by_svtuner"]),
        ("Endpoint\nevaluation", "PASS", "#5A8F29"),
    ]
    for idx, (x, (header, body, color)) in enumerate(zip(xs, labels), start=1):
        ax.scatter([x], [0.66], s=245, facecolors="white", edgecolors=color, linewidths=1.25, zorder=3)
        ax.text(x, 0.66, str(idx), ha="center", va="center", fontsize=8.0, fontweight="bold", color=color, zorder=4)
        ax.text(x, 0.46, header, ha="center", va="center", fontsize=6.4, fontweight="bold", color=PALETTE["text"], linespacing=1.05)
        ax.text(x, 0.29, body, ha="center", va="center", fontsize=5.4, color="#4D4D4D")
    for start, end in [
        ((0.18, 0.66), (0.29, 0.66)),
        ((0.43, 0.66), (0.54, 0.66)),
        ((0.68, 0.66), (0.79, 0.66)),
    ]:
        ax.add_patch(FancyArrowPatch(start, end, arrowstyle="-|>", mutation_scale=9, lw=1.0, color="#6E6E6E"))
    guardrails = [
        ("endpoint locked", 0.12),
        ("no endpoint tuning", 0.34),
        ("no rerun", 0.58),
        ("no threshold change", 0.84),
    ]
    for text, x in guardrails:
        pill = FancyBboxPatch((x - 0.077, 0.06), 0.154, 0.075, boxstyle="round,pad=0.008,rounding_size=0.016", fc="#F7F7F7", ec="#BDBDBD", lw=0.62)
        ax.add_patch(pill)
        ax.text(x, 0.097, text, ha="center", va="center", fontsize=4.6, color="#4D4D4D")
...
def create_figures(df: pd.DataFrame, phase8: dict[str, Any], img: np.ndarray, width: float, height: float, lowres: float) -> tuple[tuple[float, float, float, float], tuple[float, float, float, float], list[dict[str, Any]]]:
    ...
    save_all(fig, "fig_bioapp_main_composite_v3_11")

    for stem, drawer in [
        ("fig_bioapp_panel_A_design_schematic_v3_11", lambda axis: panel_a(axis)),
        ("fig_bioapp_panel_B_endpoint_concordance_plot_v3_11", lambda axis: panel_b(axis, phase8)),
        ("fig_bioapp_panel_C_burden_reduction_v3_11", lambda axis: panel_c(axis, phase8)),
    ]:
        fig, ax = plt.subplots(figsize=(4.1, 1.65), facecolor="white")
        drawer(ax)
        save_all(fig, stem)
    ...
    save_all(fig, "fig_bioapp_panel_D_whole_section_tissue_overlay_v3_11")

    fig = plt.figure(figsize=(8.3, 2.55), facecolor="white")
    gs = GridSpec(1, 1)
    panel_d(fig, gs[0, 0], df, img, width, height, show_d2d3_endpoint_reference=False)
    save_all(fig, "fig_bioapp_panel_D_whole_section_tissue_overlay_v3_11_no_external_markers_in_D2_D3")

    for stem, crop, label, title in [
        ("fig_bioapp_panel_E_local_fov1_tissue_overlay_v3_11", fov1, "E", "Local field of view 1"),
        ("fig_bioapp_panel_F_local_fov2_tissue_overlay_v3_11", fov2, "F", "Local field of view 2"),
    ]:
    ...
    old_main = ROOT / "visualizations" / "bioapp_experiment" / "bioapp_main_figure_v3_10_d2_light_green_footprint_correction" / "fig_bioapp_main_composite_v3_10.png"
    new_main = OUT / "fig_bioapp_main_composite_v3_11.png"
    if old_main.exists() and new_main.exists():
        ...
        for output_name, title, crop_top in [
            ("fig_bioapp_v3_11_full_before_after_comparison.png", "Full figure before/after", False),
            ("fig_bioapp_v3_11_ABC_before_after_comparison.png", "Top-row A/B/C before/after", True),
        ]:
            ...
            axes[0].set_title("Before: V3.10", fontsize=8)
            axes[1].imshow(after)
            axes[1].set_title("After: V3.11", fontsize=8)
...
def create_d2d3_optional_outputs(df: pd.DataFrame, img: np.ndarray, width: float, height: float) -> None:
    ...
    fig.savefig(OUT / "fig_bioapp_panel_D1_endpoint_v3_11.png", dpi=600, bbox_inches="tight")
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(3.2, 3.1), facecolor="white")
    draw_forced_call_footprint_panel(ax, df, img, width, height, "D2 Baseline forced-call footprint", refined_d_panel=True)
    fig.savefig(OUT / "fig_bioapp_panel_D2_baseline_forced_call_footprint_v3_11.png", dpi=600, bbox_inches="tight")
    fig.savefig(OUT / "fig_bioapp_panel_D2_baseline_forced_call_footprint_v3_11.pdf", bbox_inches="tight")
    fig.savefig(OUT / "fig_bioapp_panel_D2_baseline_forced_call_footprint_v3_11.svg", bbox_inches="tight")
    plt.close(fig)
    ...
    fig.savefig(OUT / "fig_bioapp_panel_D3_svtuner_withheld_score_v3_11.png", dpi=600, bbox_inches="tight")
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(3.2, 3.1), facecolor="white")
    draw_binary_panel(ax, df, img, width, height, "D4 Endpoint + withheld overlap")
    add_overlap_legend(ax, local=False)
    fig.savefig(OUT / "fig_bioapp_panel_D4_endpoint_withheld_overlap_v3_11.png", dpi=600, bbox_inches="tight")
    plt.close(fig)
    ...
    comparisons = [
        (
            ...
            OUT / "fig_bioapp_panel_D2_baseline_forced_call_footprint_v3_11.png",
            "Before: V3.9 gray D2 footprint",
            "After: V3.11 D2 preserved",
            "fig_bioapp_v3_11_D2_before_after_comparison.png",
        ),
        (
            ...
            OUT / "fig_bioapp_panel_D4_endpoint_withheld_overlap_v3_11.png",
            "Before: V3.9 D4 overlap",
            "After: V3.11 D4 preserved",
            "fig_bioapp_v3_11_D4_before_after_comparison.png",
        ),
        (
            ...
            OUT / "fig_bioapp_panel_D_whole_section_tissue_overlay_v3_11.png",
            "Before: V3.9 green D3",
            "After: V3.11 D preserved",
            "fig_bioapp_v3_11_D_before_after_comparison.png",
        ),
    ]
...
def write_reports(
    ...
    visualization_report = {
        ...
        "panel_A_issue_fixed": ["text overlap removed", "experimental design logic expanded"],
        "panel_B_metric_cards_removed": True,
    ...
    write_json(OUT / "fig_bioapp_v3_11_d2_distribution_report.json", d2_distribution)
    write_json(OUT / "fig_bioapp_v3_11_visualization_report.json", visualization_report)
    alignment = {
        "phase": "BioApp Main Figure V3.11 top-row A/B/C redesign",
    ...
    write_json(OUT / "fig_bioapp_v3_11_tissue_overlay_alignment_report.json", alignment)
    ...
    write_json(OUT / "fig_bioapp_v3_11_palette.json", palette_out)
    write_rows(
        OUT / "fig_bioapp_v3_11_fov_crop_report.csv",
        crop_rows,
    ...
    guardrails = {
        ...
        "panel_A_design_logic_expanded": True,
        "panel_B_metric_cards_removed": True,
        "panel_B_replaced_by_endpoint_concordance_plot": True,
        "panel_C_style_harmonized": True,
        "D_E_F_preserved": True,
        "d2_footprint_color_changed_to_light_green": True,
        "d1_marker_size_reduced": True,
        "d3_colormap_restored_to_blue": True,
    ...
    write_json(OUT / "fig_bioapp_v3_11_visualization_guardrails.json", guardrails)
...
"""
    (OUT / "fig_bioapp_v3_11_layout_report.md").write_text(report, encoding="utf-8")
    readme = f"""BioApp Main Figure V3.11 top-row A/B/C redesign completed.
Decision: {phase_decision}
Panel A redesigned: true
Panel B metric cards removed: true
Panel B replaced by endpoint-concordance plot: true
Panel C refined: true
D/E/F preserved: true
Data changed: false
Metrics changed: false
Endpoint redefined: false
Main figure: fig_bioapp_main_composite_v3_11.pdf
Next: visual review
"""
...
def main() -> None:
    ...
    guardrails = read_json(OUT / "fig_bioapp_v3_11_visualization_guardrails.json")
    vis_report = read_json(OUT / "fig_bioapp_v3_11_visualization_report.json")
    print("BioApp Main Figure V3.11 completed.")
    print(f"Decision: {guardrails['decision']}")
    print(f"Panel A overlap fixed: {str(guardrails['panel_A_text_overlap_removed']).lower()}")
    print(f"Panel B metric cards removed: {str(guardrails['panel_B_metric_cards_removed']).lower()}")
    print(f"D/E/F preserved: {str(guardrails['D_E_F_preserved']).lower()}")
