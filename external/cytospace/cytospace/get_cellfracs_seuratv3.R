...
get_cellfracs_seuratv3 <- function(sc_path, ct_path, st_path, outdir, prefix, disable_downsampling = FALSE){
    ...
    st[is.na(st)] <- 0
    # In some environments SCTransform(ncells = NULL) can hit a split() edge case.
    # Explicitly cap ncells to avoid a zero-length grouping vector.
    ncells_st <- min(ncol(st), 5000)
    suppressMessages(st <- CreateSeuratObject(st) %>% SCTransform(verbose = FALSE, ncells = ncells_st) %>% RunPCA())
