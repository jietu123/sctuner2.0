#!/usr/bin/env python
"""Run and audit a full Stage3A + Stage3B composite noise benchmark."""
...
def configure_noise(noise_fraction: float, noise_seed: int) -> None:
    global NOISE_FRACTION, NOISE_SEED, NOISE_PERCENT, NOISE_TAG, PREFIX
    global SUFFIX, STAGE3_DIR, STAGE3B_DIR, STAGE4_DIR, NOISE_SUFFIX, OUT_REL

    percent = noise_fraction * 100.0
    rounded = int(round(percent))
    if not (0.0 < noise_fraction < 1.0):
        raise ValueError("--noise_fraction must be between 0 and 1")
    if abs(percent - rounded) > 1e-9 or not (0 < rounded < 100):
        raise ValueError("The benchmark requires an integer noise percentage between 1 and 99")

    NOISE_FRACTION = float(noise_fraction)
    NOISE_SEED = int(noise_seed)
    NOISE_PERCENT = rounded
    NOISE_TAG = f"scnoise{rounded:02d}"
    PREFIX = f"composite_{NOISE_TAG}_stage3ab_full"
    SUFFIX = f"_{NOISE_TAG}_stage3ab_full"
    STAGE3_DIR = f"stage3_typematch{SUFFIX}"
    STAGE3B_DIR = f"stage3b_st_unsupported{SUFFIX}"
    STAGE4_DIR = f"stage4_cytospace{SUFFIX}"
    NOISE_SUFFIX = f"_{NOISE_TAG}"
    OUT_REL = Path("visualizations/method_comparison") / PREFIX


def parse_args(default_noise_fraction: float = 0.10) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    # Resolve from cwd on Windows; some isolated Conda builds misdecode non-ASCII __file__ paths.
    parser.add_argument("--project_root", default=".")
    parser.add_argument("--python", default=sys.executable)
    parser.add_argument("--external_python", default=sys.executable)
    parser.add_argument("--noise_fraction", type=float, default=default_noise_fraction)
    parser.add_argument("--noise_seed", type=int, default=42)
    parser.add_argument("--prepare_inputs", action="store_true")
    parser.add_argument("--overwrite_inputs", action="store_true")
...
def prepare_noise_inputs(
    ...
    for scenario in SCENARIOS:
        summary_path = root / "result" / scenario.sample / "sc_noise_generation_summary.json"
        cfg_path = root / "configs" / "datasets" / f"{scenario.sample}.yaml"
        truth = truth_path(root, scenario)
        if resume and summary_path.exists() and cfg_path.exists() and truth.exists():
            summary = read_json(summary_path)
            if (
                summary.get("source_sample") == scenario.base_sample
                and summary.get("target_sample") == scenario.sample
                and abs(float(summary.get("noise_fraction", -1)) - NOISE_FRACTION) < 1e-12
                and int(summary.get("seed", -1)) == NOISE_SEED
            ):
                continue
        command = [
            python,
            str(root / "scripts" / "generate_sc_noise_from_processed_sim.py"),
            "--project_root", str(root),
            "--sim_group", scenario.group,
            "--source_sample", scenario.base_sample,
            "--target_sample", scenario.sample,
            "--noise_fraction", str(NOISE_FRACTION),
            "--seed", str(NOISE_SEED),
        ]
        if overwrite:
            command.append("--overwrite")
        run_command(command, logs / f"{scenario.sample}_noise_generation.log", root)


def run_cytospace_baselines(
    root: Path,
    python: str,
    out_dir: Path,
    resume: bool,
    ...
    for scenario in SCENARIOS:
        summary_path = (
            root / "result" / scenario.sample / "stage4_cytospace_baseline"
            / "cytospace_output" / "stage4_summary.json"
        )
        ...
        command = [
            ...
            "--n_processors", str(n_processors),
            "--n_subspots", str(n_subspots),
            "--mapping_cells_per_spot", str(mapping_cells_per_spot),
        ]
        run_command(command, logs / f"{scenario.sample}_cytospace_baseline.log", root)
...
def run_external_methods(
    ...
    command = [
        external_python,
        str(root / "scripts" / "run_nine_scenario_method_benchmark.py"),
        "--project_root", str(root),
    ...
    run_command(command, out_dir / "execution_logs" / f"{PREFIX}_external_methods.log", root)
...
def input_and_reuse_audit(root: Path, out_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for scenario in SCENARIOS:
        ...
        deterministic_recreation = bool(
            ...
            and abs(float(noise_summary.get("noise_fraction", -1)) - NOISE_FRACTION) < 1e-12
            and int(noise_summary.get("seed", -1)) == NOISE_SEED
            ...
            reuse_rows.append(
                {
                    ...
                    "evidence": (
                        ...
                        + ("output postdates noisy input" if postdates_noise else
                           f"input was deterministically recreated later from the same source with "
                           f"p={NOISE_FRACTION:g} and seed={NOISE_SEED}")
                    ),
                }
            )
    input_df = pd.DataFrame(input_rows)
    reuse_df = pd.DataFrame(reuse_rows)
    input_df.to_csv(out_dir / f"{PREFIX}_input_audit.csv", index=False)
    reuse_df.to_csv(out_dir / f"{PREFIX}_reused_method_audit.csv", index=False)
...
def scenario_manifest(root: Path, out_dir: Path) -> pd.DataFrame:
    ...
    frame.to_csv(out_dir / f"{PREFIX}_scenario_manifest.csv", index=False)
    return frame
...
def stage_audits(root: Path, out_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    stage3a_df.to_csv(out_dir / f"{PREFIX}_stage3a_audit.csv", index=False)
    stage3b_df.to_csv(out_dir / f"{PREFIX}_stage3b_audit.csv", index=False)
...
def evaluate(root: Path, out_dir: Path, reuse_df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    plot(
        scenario_df,
        out_dir / f"fig1d_{NOISE_TAG}_stage3ab_full.png",
        out_dir / f"fig1d_{NOISE_TAG}_stage3ab_full.pdf",
        f"Spatial cell-type composition recovery, {NOISE_PERCENT}% sc-reference noise",
...
def main(default_noise_fraction: float = 0.10) -> int:
    args = parse_args(default_noise_fraction)
    configure_noise(args.noise_fraction, args.noise_seed)
    # Do not resolve a subst drive back to its non-ASCII physical path.
    root = Path(args.project_root).absolute()
    out_dir = root / OUT_REL
    out_dir.mkdir(parents=True, exist_ok=True)
    if args.prepare_inputs:
        prepare_noise_inputs(root, args.python, out_dir, args.resume, args.overwrite_inputs)
    if args.run_baselines:
        run_cytospace_baselines(
            root,
            args.python,
            out_dir,
            args.resume,
    ...
    if args.run_external:
        run_external_methods(root, args.external_python, out_dir, args.force_external)
    ...
    (out_dir / f"{PREFIX}_guardrails.json").write_text(json.dumps(guardrails, indent=2), encoding="utf-8")
    summary = {
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "decision": guardrails["decision"],
        "route": "Stage3A unsupported-reference filtering + Stage3B blanking + CytoSPACE",
        "noise": (
            f"{NOISE_PERCENT}% independent entry-wise within-gene replacement, "
            f"seed {NOISE_SEED}"
        ),
