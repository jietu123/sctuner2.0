...
│
└─ 模拟场景流水线：sample = S0_matched ...（每个场景跑一次）
...
**目标：**
 用真实 BRCA scRNA + 类型注释做模板，**单独构建带真值的模拟场景**（S0 / M2…），为后面的 Stage5“有真值评估”提供输入。
...
2. **给 World 细胞设计空间布局：**
   ...
   - 确保：
     - S0 场景：各类型都有合理的支持区域；
     - M2 场景：人为制造 “sc 缺 type / ST 缺 type / 稀有型” 等情况。
3. **构造模拟 ST spot：**
...
4. **多场景生成：**
   - 针对每个 `scenario_id`（S0 / M2…）重复 1–3 步；
   - 保证：
...
在 M2 的配置里，只需修改 `sc_missing_types` / `st_missing_types` 等。
...
1. **模块一（SVG+HVG 动态加权 / Step1）到底有没有用？**
   ...
   - 在 M2 等复杂场景下：plus 是否仍然能保持较好的 SVG 重构，而不过度损害全局表达拟合和类型分布？
...
- `T_missing`：被刻意设为“缺型”的类型（对 M2 分别含义不同）；
- `T_rare`：被设为“稀有型”的类型。
...
> 在 M2 场景中，Stage5 会比较 baseline vs plus 的这些指标，验证类型插件是否达到了“宁可 Unknown，也不要瞎扩散”的设计目的。
...
- 对相同 `(config_id, svg_refine_lambda, enable_refine)` 在不同场景中的表现做聚合：
  - 看某配置是否在“S0 + M2 …”中都表现不错，还是只在某类场景好；
    ...
    "recommended_configs": [
      {
        ...
        "suggested_seeds": 3,
        "notes": "S0/M2 场景中 SVG 指标显著优于 baseline，类型/Unknown 行为合理，稳定性良好，cost/约束变化可接受。"
      },
      {
        "config_id": "svg_cost_v1",
        "enable_refine": false,
...
3. **阶段 5（模拟数据验证）的输出**
   ...
   - 每个模拟场景（S0/M2…）、每个 backend、每种模式（baseline / plus）的：
...
- `Table4_simulation_eval.csv`
  - 每个场景（S0/M2）、每个 backend、每个配置（若有）的真值指标；
  - 增加列：
    - `config_id`；
...
- 图像内容：
  ...
  - `Fig4c_sim_multi_run_score_vs_config.png`（新加，Step3 的核心一部分）
    - 场景 S0/M2 的多个指标综合成一个 `overall_sim_score`；
    - 每个 config（seed / λ 组合）一个点或一条柱；
    - 标出 canonical 配置；
    - 说明：
      - canonical 不是某个“离群点”，而是在多个配置中取得 **兼顾 S0 准确、M2 稳定** 的高分解。
- 表格内容：
  - `Table4_simulation_eval.csv`：
...
- 处理几类关键维度：
  ...
  - **sample**：真实样本（`real_brca`） vs 模拟场景（`S0_matched` /  等）；
  - **stage**：0–7 + SimGen；
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default="real_brca",
        help="要跑的样本 / 场景列表，逗号分隔，如 real_brca,S0_matched",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="只生成模拟场景，而不跑完整 pipeline，如 S0_matched",
    )
...
- 模拟线有真值，是最安全的地方：
  - 我们可以大胆试多组 `(配置, seed, λ)`；
  - 然后用 S0/M2 的主指标来做**系统的 config selection**，
...
4. **打分 & 选出“模拟线最佳配置”**
   ...
   - 对于通过硬筛选的配置 c，定义一个综合分数，例如：
     ...
     - $$
       ...
       - w_{miss} \cdot \Delta wrong_{miss}(c)
